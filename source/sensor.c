...
#ifdef CY_IMU_I2C_MPU6050
    /* MPU6050 driver structures */
    mtb_mpu6050_data_t data;
...
cy_rslt_t sensor_init(void)
{
    ...
    MPU6050_Initialize(&mMPU,&i2c, MPU6050_DEFAULT_ADDRESS);
    /*if(CY_RSLT_SUCCESS != result)
    {
        return result;
    }
...
void sensor_interrupt_handler(void *callback_arg, cyhal_timer_event_t event)
{
	...
	MPU6050_GetRawAccelGyro(&mMPU,buffer);
	//printf("A]%hd:%hd:%hd\r\n", buffer[0],buffer[1],buffer[2]);
	//printf("G]%hd:%hd:%hd\r\n", buffer[3],buffer[4],buffer[5]);

	data.accel.x=buffer[0];
	data.accel.y=buffer[1];
	data.accel.z=buffer[2];

	data.gyro.x=buffer[3];
	data.gyro.y=buffer[4];
	data.gyro.z=buffer[5];
